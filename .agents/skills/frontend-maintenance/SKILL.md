...
1. Read `@.agents/project-context.md` and `AGENTS.md` for current static-site constraints
...
|-------|----------|-------|
| Structure | `index.html` | Section IDs are anchor targets |
| Styles | `assets/css/` | Reuse variables from `tokens.css`; place rules in the matching module |
| Behavior | `assets/js/site.js` | Language toggle, menu dialog, observers |
...
- `openMenu()` / `toggleMenu()` / `closeMenu()` — mobile dialog navigation and focus restoration
...
| Avoid | Why |
|-------|-----|
| Adding a framework, package manager, or bundler without request | Breaks current zero-build setup |
